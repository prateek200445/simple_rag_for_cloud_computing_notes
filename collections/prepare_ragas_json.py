...
# 🔹 Load your retrieval results file (already saved after RAG retrieval)
...
for idx, item in enumerate(data[:10], start=1):
    ...
    # Generate the model's RAG-style response
    try:
        response = model.generate_content(prompt)
        generated_answer = response.text.strip()
    except Exception as e:
        print(f"⚠️ Error generating answer for Q{idx}: {e}")
        generated_answer = ""

    # Append in RAGAS format
    ragas_data.append({
        "question": question,
        "contexts": contexts,
        "answer": ground_truth,
        "generated_answer": generated_answer,
        "reference": ground_truth   # ✅ Added for RAGAS compatibility
    })
